...
if ( ! class_exists( 'Kirki_Controls_Code_Control' ) ) {
	...
	class Kirki_Controls_Code_Control extends Kirki_Customize_Control {
		...
		public function enqueue() {
			...
			wp_enqueue_script( 'kirki-code', trailingslashit( Kirki::$url ) . 'assets/js/controls/code.js', array(
				'jquery',
				'codemirror'
			), false );
			...
			if ( in_array( $this->choices['language'], array( 'html', 'htmlmixed' ) ) ) {
				...
				wp_enqueue_script( 'codemirror-language-php', trailingslashit( Kirki::$url ) . 'assets/js/vendor/codemirror/mode/php/php.js', array(
					'jquery',
					'codemirror'
				) );
			} else {
				// Add language script.
				wp_enqueue_script( 'codemirror-language-' . $this->choices['language'], trailingslashit( Kirki::$url ) . 'assets/js/vendor/codemirror/mode/' . $this->choices['language'] . '/' . $this->choices['language'] . '.js', array(
					'jquery',
					'codemirror'
				) );
			}
